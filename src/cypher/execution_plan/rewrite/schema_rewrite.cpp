...
std::vector<cypher::SchemaGraphMap> SchemaRewrite::GetEffectivePath(const lgraph::SchemaInfo &schema_info,cypher::SchemaNodeMap* schema_node_map,cypher::SchemaRelpMap* schema_relp_map){
    //根据schema构建目标图
    ...
    while((schema=e_schema_manager.GetSchema(label_num))){
        ...
        for(auto pair:ec ){
            ...
            target_graph.AddEdge(e_cnt,(size_t)GetLabelNum(pair.first),(size_t)GetLabelNum(pair.second),label_nums,parser::LinkDirection::LEFT_TO_RIGHT);
            // std::cout<<"edge id :"<<e_cnt<<",label num:"<<GetLabelNum(schema->GetLabel())<<",src id:"<<(size_t)GetLabelNum(pair.first)<<",dst id:"<<(size_t)GetLabelNum(pair.second)<<std::endl;
            e_cnt++;
        }
        label_num++;
    }
...
void SchemaRewrite::MatchRecursive(size_t vid,size_t t_vid){
    if(depth==query_size){
        ...
        #ifdef DEBUG
            std::cout<<"num of stateinfos:"<<candidate_state_infos.size()<<",depth:"<<depth<<std::endl;
        ...
        for(StateInfo si:candidate_state_infos){
            for(auto it=si.m_id_map.begin();it!=si.m_id_map.end();it++){
                #ifdef DEBUG
                    std::cout<<"check:query:"<<si.m_next_vid<<",target:"<<it->first<<std::endl;
                #endif
        ...
        if(core_2[i]>-1){
            ...
            for(size_t eid:node.m_outedges){
                ...
                if(core_2[edge.m_target_id]==-1){
                    ...
                    std::map<size_t,std::set<size_t>> id_map=GetNextTVidByEdgeId(edge_ids,core_2[i]);
...
//根据目标图上的边id集合获取点id集合
std::map<size_t,std::set<size_t>> SchemaRewrite::GetNextTVidByEdgeId(std::set<size_t>& edge_ids,size_t t_vid){
...
void SchemaRewrite::AddMapping(){
    ...
    for(Edge e:query_graph.m_edges){
        ...
        if(srm_it!=srm.end()){
            ...
            srm_it->second=std::tuple<cypher::NodeID,cypher::NodeID,std::set<std::string>,parser::LinkDirection>(std::get<0>(value),std::get<1>(value),edge_labels,std::get<3>(value));
        }
    }
...
void SchemaRewrite::PrintMapping(){
    std::cout<<"Node Mapping:"<<std::endl;
        ...
        for(Edge e:query_graph.m_edges){
            ...
            std::cout<<"("<<src_id<<"["<<src_label<<"])-["<<label_str<<"]-("<<tar_id<<"["<<tar_label<<"]) "<<std::endl;
        }
}
